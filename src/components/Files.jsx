...
const Files = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      fetch(`${import.meta.env.VITE_REACT_APP_API_ENDPOINT}/files`, {
        headers: {
...
const File = ({ folderName, files }) => {
  ...
  const downloadFile = async (file) => {
    let url = await fetch(`${import.meta.env.VITE_REACT_APP_API_ENDPOINT}/files?folder=${folderName}&file=${file}`, {
      headers: {
        "x-api-key": import.meta.env.VITE_REACT_APP_API_KEY,
      },
    })
